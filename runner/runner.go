...
type Runner struct {
	...
	isConsoleLog         bool          // if set to true then the shell's commands output is printed on console. default=false
	...
	successCriteria      string        // if this string is found the in the log then that means the output is vefified, and now the status becomes SUCCESSFUL. default="" that means verification is disabled and now the status can become FINISHED
	...
	cmd                  *exec.Cmd     // This is the command variable from exec package. Internal use
	exitCode             int           // exit code of the system call
	...
	onSuccessCallback    func(*Runner) // This function gets called when the system call is execuited and the verification phrase is found in the stdout
	onCompleteCallback   func(*Runner) // This function gets called when the system call execution completes with exit code '0'. The output of the system call will be injected in the callback function
	onFailCallback       func(*Runner) // This function get called when the system call exits with a non-zero exit code. The stdout of the system call is injected into the callback function
	onTimeoutCallback    func(*Runner) // This function gets called when the system call took more than the defined timeout duration.
}
...
func (r *Runner) DisableConsole() {
	r.isConsoleLog = false
}

func (r *Runner) SetTimeout(timeout int, callback ...func(*Runner)) {
	r.timeout = timeout
	if len(callback) > 0 {
		r.onTimeoutCallback = callback[0]
	}
}
...
func (r *Runner) SetSuccessCriteria(phrase string, callback ...func(*Runner)) {
	r.successCriteria = phrase
	if len(callback) > 0 {
		r.onSuccessCallback = callback[0]
	}
}
...
func (r *Runner) SetOnCompleteCallback(callback func(*Runner)) {
...
func (r *Runner) SetOnFailCallback(callback func(*Runner)) {
	r.onFailCallback = callback
}

// get runner settings.
func (r *Runner) GetConfig() map[string]interface{} {
	var logSize = len(r.logBuffer)
	return map[string]interface{}{
		"id":                  r.id,
		"console-log":         r.isConsoleLog,
		"timeout":             r.timeout,
		"waiting-period":      r.waitingPeriod,
		"log-file":            r.logFile,
		"execuited-at":        r.execuitedAt,
		"execution-time-nano": r.executionTimeNano,
		"success-criteria":    r.successCriteria,
		"status":              r.status,
		"system-command":      r.sysCmd,
		"log-size-bytes":      logSize,
...
func (r *Runner) Execute(commands ...string) ([]byte, error) {
	...
	go func() {
		...
		err = r.cmd.Wait()
		if err != nil {
			// Check if the error is due to command exiting with a non-zero status
	...
	// start an async timer. when time runs out, the timer will send `timeout` channel will get a signal. `case <-timeout:` will be activated
	...
	case <-timeout:
		...
		if r.cmd.Process != nil && r.timeout > 0 {
	...
	if r.status == "TIMEDOUT" {
		...
		if r.onTimeoutCallback != nil {
			r.onTimeoutCallback(r) // Invoke the user-defined success callback
		}
		return r.logBuffer, nil
	}

	// search and match output and set status to VERIFIED
	if r.successCriteria != "" && strings.Contains(string(r.logBuffer), r.successCriteria) {
		// if attached, then invoke the onSuccessCallback() function
		if r.onSuccessCallback != nil {
			r.onSuccessCallback(r) // Invoke the user-defined success callback
		}
		r.status = "SUCCEEDED"
		return r.logBuffer, nil
	}
	...
	if r.exitCode == 0 {
		...
		if r.onCompleteCallback != nil {
			r.onCompleteCallback(r) // callbacl
		}
		return r.logBuffer, nil
	}
	...
	if r.onFailCallback != nil {
		r.onCompleteCallback(r) // callback
	}
	return r.logBuffer, nil
}
